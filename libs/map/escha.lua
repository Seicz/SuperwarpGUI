...
return T{
    ...
    missing = function(warpdata, zone, p)
        ...
        if zone == 288 then zname = 'Escha Zi\'tah' end
        if zone == 289 then zname = 'Escha Ru\'an' end
        if zone == 291 then zname = 'Reisenjima' end
        zd = zname and warpdata[zname]
        if zd == nil then
            return nil, 'You cannot check missing destinations from here.'
        end

        -- fork fix: upstream appends `z..'-'..d` but `z` is undefined here.
